...
class APIMonitor:
    ...
    async def start_monitoring(self) -> None:
        ...
        logger.info(f"Starting API monitoring for {self.api_base_url}")
    ...
    async def _monitor_endpoints(self) -> None:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"Error monitoring {self.endpoints[i]}: {result}")
                # Create error metric
    ...
    async def _check_endpoint(self, endpoint: str) -> None:
        ...
        try:
            async with self.session.get(url) as response:
                ...
                logger.debug(f"Checked {endpoint}: {response.status} ({response_time:.3f}s)")
