...
class MonitoringAgent:
            ...
            for endpoint, count in endpoint_counts.items():
                if count >= 3:  # Multiple slow requests
                    alert = Alert(
                        ...
                        description=f"Endpoint {endpoint} has {count} slow responses (>{self.response_time_threshold}s) in recent measurements. Average response time: {avg_response_time:.2f}s",
                        endpoint=endpoint,
    ...
    def _analyze_error_rates(self, metrics: List[MetricData]) -> None:
        ...
        if error_rate > self.error_rate_threshold:
            ...
            alert = Alert(
                ...
                description=f"Error rate is {error_rate:.1%} (threshold: {self.error_rate_threshold:.1%}). Total errors: {len(error_requests)}/{total_requests}",
    ...
    def _detect_anomalies(self, metrics: List[MetricData]) -> None:
        ...
        if outliers:
            alert = Alert(
                ...
                timestamp=datetime.now(),
                level=AlertLevel.WARNING,
                title=f"Response Time Anomalies Detected",
                description=f"Found {len(outliers)} anomalous response times. Mean: {mean_rt:.2f}s, StdDev: {std_rt:.2f}s",
    ...
    def _generate_insights(self, metrics: List[MetricData]) -> List[str]:
        ...
        insights.append(f"Best performing endpoint: {best_endpoint[0]} ({best_endpoint[1]:.2f}s avg)")
        insights.append(f"Slowest endpoint: {worst_endpoint[0]} ({worst_endpoint[1]:.2f}s avg)")
